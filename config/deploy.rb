...
set :keep_releases, 5
set :application, "Starhubmonittest"
set :repository,  "git@github.com:dangluan/monittest.git"
set :scm, :git
...
namespace :deploy do
  ...
  task :restart, :roles => :web do
    if remote_file_exists?("#{shared_path}/pids/monittest.pid")
      run "kill -s QUIT `cat #{shared_path}/pids/monittest.pid`"
    end
  ...
  namespace :assets do
    task :precompile do            
      if !(ENV["SKIP_ASSET"] == "true")      # can be use cap deploy SKIP_ASSET=true  
...
#   desc "Start Faye"
#   task :start do
#     run "cd #{current_path} && bin/rackup faye.ru -s thin -E #{rails_env} -D --pid #{shared_path}/pids/faye.pid"
#   end
#   
#   desc "Stop Faye"
#   task :stop do
#     if remote_file_exists?("#{shared_path}/pids/faye.pid")
#       run "kill -9 `cat #{shared_path}/pids/faye.pid`"
#     end
#   end
# end
